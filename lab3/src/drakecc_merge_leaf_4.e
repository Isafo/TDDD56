...
# 36 "presort.c"
static array_int_t *local;

int
presort_init_leaf_4(task_t *task, void* aux)
{
 link_t *link;
 array_int_t *tmp;
 ...
 if(args->argc > 0)
 {
  ...
  if(tmp != ((void *)0))
  {
   ...
   chunk_size = input_size / ((drake_task_number_merge() + 1) / 2);


   local = pelib_array_int_loadfilenamewindowbinary(input_filename, chunk_size * (leaf_index - 1), chunk_size);
  }
  else
  {
   fprintf(stderr, "[%s:%s:%d:P%zu:%s] Cannot open input file \"%s\". Check application arguments.\n", "presort.c", __FUNCTION__, 81, drake_platform_core_id(), task->name, input_filename);
  }
 }
 else
 {
  fprintf(stderr, "[%s:%s:%d:P%zu:%s] Missing file to read input from. Check application arguments.\n", "presort.c", __FUNCTION__, 86, drake_platform_core_id(), task->name);
 }
...
int
presort_start_leaf_4(task_t *task)
{
 link_t *link;
 int j;


 sort((int*)local->data, pelib_array_int_length(local));
...
int
presort_run_leaf_4(task_t *task)
{
 ...
 pelib_cfifo_int_fill(parent_link->buffer, parent_size < left ? parent_size : left);
...
int
presort_kill_leaf_4(task_t *task)
{
...
int
presort_destroy_leaf_4(task_t *task)
{
